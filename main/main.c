...
int php_module_startup(sapi_module_struct *sf, zend_module_entry *additional_modules, uint32_t num_additional_modules)
{
	...
	module_shutdown = 0;
	module_startup = 1; // 全局变量, 标识已经开始执行sapi的初始化操作
	sapi_initialize_empty_request();
	sapi_activate(); // 激活sapi, 初始化

	if (module_initialized) { // 防止重复操作
		return SUCCESS;
	}
...
#else
	memset(&core_globals, 0, sizeof(core_globals));
	php_startup_ticks(); // 初始化定时任务队列
#endif
	gc_globals_ctor(); // gc 垃圾回收相关
	// 提供给zend引擎使用的各种函数句柄(回调)
	...
	zend_startup(&zuf, NULL); // zend引擎初始化 php解释器 opcode相关 (通过zuf传递各种句柄给zend引擎)
	...
	/* Register constants */ // 注册各种常量  EG(zend_constants) hashtable  get_defined_constants()可以查看
	...
	php_binary_init(); // 查找php/php-fpm可执行程序的绝对路径
	if (PG(php_binary)) { // 找到之后, 添加到常量hashtable
	...
	php_output_register_constants(); // 注册输出常量
	php_rfc1867_register_constants(); // 文件上传相关的一些常量

	/* this will read in php.ini, set up the configuration parameters,
	   load zend extensions and register php function extensions
	   to be loaded later */ 
	// important!!!! 解析php.ini配置文件
	...
	zend_set_utility_values(&zuv); // zuv传给zend引擎
	php_startup_sapi_content_types();

	/* startup extensions statically compiled in 开启静态编译的PHP核心扩展 */
	 ...
	 */
	php_ini_register_extensions(); // 加载zend/php扩展
	...
	php_disable_functions(); // 根据配置禁用 指定的方法
	php_disable_classes(); // 禁用类

	/* make core report what it should */ // 为core模块设置2个参数
	...
	module_initialized = 1; // 标识符, 标识所有扩展已经执行
