...
static void php_ini_parser_cb(zval *arg1, zval *arg2, zval *arg3, int callback_type, HashTable *target_hash)
{
	...
	// 根据 callback_type 来判断php.ini配置项的类型
	switch (callback_type) {
		case ZEND_INI_PARSER_ENTRY: {
				...
				if (!is_special_section && !strcasecmp(Z_STRVAL_P(arg1), PHP_EXTENSION_TOKEN)) { /* load PHP extension */
					...
					zend_llist_add_element(&extension_lists.functions, &extension_name); // php扩展名添加到对应的hashtable
				} else if (!is_special_section && !strcasecmp(Z_STRVAL_P(arg1), ZEND_EXTENSION_TOKEN)) { /* load Zend extension */
					extension_name = estrndup(Z_STRVAL_P(arg2), Z_STRLEN_P(arg2));
					zend_llist_add_element(&extension_lists.engine, &extension_name); // zend引擎扩展
...
int php_init_config(void)
{
	...
	zend_llist_init(&extension_lists.engine, sizeof(char *), (llist_dtor_func_t) free_estring, 1); // zend扩展, 比如:Zend OPcache
	zend_llist_init(&extension_lists.functions, sizeof(char *), (llist_dtor_func_t) free_estring, 1); // php扩展
	...
	if (!sapi_module.php_ini_ignore || sapi_module.php_ini_path_override) {
		...
		if (php_ini_file_name && php_ini_file_name[0]) {
			...
			if (!VCWD_STAT(php_ini_file_name, &statbuf)) {
				if (!((statbuf.st_mode & S_IFMT) == S_IFDIR)) {
					fh.handle.fp = VCWD_FOPEN(php_ini_file_name, "r"); // 打开配置文件
					if (fh.handle.fp) {
		...
		/* If still no ini file found, search for php.ini file in search path */ // 兜底配置文件
		if (!fh.handle.fp) {
	...
	if (fh.handle.fp) {
		...
		// 开始解析配置文件, 解析器的回调函数 php_ini_parser_cb (重点查看该函数), 配置信息存储到 configuration_hash
	...
	/* Scan and parse any .ini files found in scan path if path not empty. */ // 如果不跳过配置并且有指定扫描路径, 扫描指定路径下所有配置信息
	...
	if (sapi_module.ini_entries) { // 使用类似: php -d date.timezone=America/Adak 添加的配置
		/* Reset active ini section */
		RESET_ACTIVE_INI_HASH();
...
{
	zend_llist_apply(&extension_lists.engine, php_load_zend_extension_cb); // 先加载各个zend扩展(因为加载php扩展时对zend扩展有依赖)
	zend_llist_apply(&extension_lists.functions, php_load_php_extension_cb); // 再加载php扩展 (对各个扩展执行回调方法php_load_php_extension_cb)
