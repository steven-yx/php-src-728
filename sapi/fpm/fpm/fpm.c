...
int fpm_init(int argc, char **argv, char *config, char *prefix, char *pid, int test_conf, int run_as_root, int force_daemon, int force_stderr) /* {{{ */
{
	...
	if (0 > fpm_php_init_main()           ||
	    ...
	    0 > fpm_conf_init_main(test_conf, force_daemon) /* php-fpm.conf配置文件解析 */ ||
	    0 > fpm_unix_init_main() /*生成守护进程*/         ||
	    0 > fpm_scoreboard_init_main()    || /* 初始化 计分板 */
...
int fpm_run(int *max_requests) /* {{{ */
{
	...
	for (wp = fpm_worker_all_pools; wp; wp = wp->next) {
		int is_parent;
		// 各个进程池 生成子进程
		// 同时会获取到监听的socket赋值到fpm_globals.listening_socket
	...
	/* run event loop forever 父进程进入该循环,后面不再执行 */
	fpm_event_loop(0);
