...
{
	// 下面的信息, 是在每个work子进程内设置的
	fpm_globals.max_requests = wp->config->pm_max_requests; // 每个进程能处理的,最大请求次数
	// 监听套接字是在处理每个work pool时候生成的
	// fpm_init()->fpm_sockets_init_main()->fpm_socket_af_inet_listening_socket()
	// ->fpm_socket_af_inet_socket_by_addr()->fpm_sockets_get_listening_socket()->
	// fpm_sockets_new_listening_socket()->socket()->bind()->listen()
	fpm_globals.listening_socket = dup(wp->listening_socket); // 监听的套接字
