...
int main(int argc, char *argv[])
{
	...
	int max_requests = 500; // 每个work进程默认处理请求的次数
	int requests = 0;
	int fcgi_fd = 0;
	...
	// 设置各种信号
	zend_signal_startup();
	// cgi_sapi_module 结构体成员的初始化赋值
	sapi_startup(&cgi_sapi_module);
	cgi_sapi_module.php_ini_path_override = NULL;
	cgi_sapi_module.php_ini_ignore_cwd = 1; // 不在当前目录搜索php.ini文件

#ifndef HAVE_ATTRIBUTE_WEAK
	fcgi_set_logger(fpm_fcgi_log);
#endif

	fcgi_init(); // 主要判断是否是fastcgi (php函数 php_sapi_name()可以判断是那种模式)
...
#endif
	// 判断php后面跟的参数  (OPTIONS中有对应的值) php_optarg:为具体的参数值
	// c 为 数组OPTIONS中的值
	while ((c = php_getopt(argc, argv, OPTIONS, &php_optarg, &php_optind, 0, 2)) != -1) {
		switch (c) {
			case 'c':
				...
				cgi_sapi_module.php_ini_path_override = strdup(php_optarg); // 指定的php配置文件路径
				break;

			case 'n':
				cgi_sapi_module.php_ini_ignore = 1; //静默模式, 忽略配置信息
				break;
			...
			case 'm': /* list compiled in modules */ // 显示所有加载的扩展 php -m
				...
				goto out; // 显示完扩展信息之后,直接退出

			case 'i': /* php info & quit */ // 与 phpinfo() 类似, 显示php信息
				php_information = 1;
				break;
			...
			case 'v': /* show php version & quit */ // 显示php版本号
				cgi_sapi_module.startup(&cgi_sapi_module);
	...
	if (php_information) { // 只有 php -i 才会走到这里
	...
	cgi_sapi_module.executable_location = argv[0]; // php命令的路径

	/* startup after we get the above ini override se we get things right */ 
	//执行php_cgi_startup()方法, 进行配置文件解析等操作
...
<a href=\"http://php.net/install.windows\">the manual page</a>.</p>\n");
	...
	fpm_is_running = 1; // fpm已经启动 标识符

	// 各个进程池生成work进程 !!! important
	// fcgi_fd 为监听的 socket
	...
	request = fpm_init_request(fcgi_fd); //开始执行请求, 请求初始化

	zend_first_try { // work进程循环执行各个请求
		while (EXPECTED(fcgi_accept_request(request) >= 0)) { // accept 等待请求到达, 阻塞
			...
			php_execute_script(&file_handle); // php解释器执行
...
out:
	...
	SG(server_context) = NULL;
	php_module_shutdown(); // 关闭各个模块, 释放内存
